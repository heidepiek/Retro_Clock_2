...
//  myGLCD.drawRoundRect(2  , 2 , 315 , 235 , 2 , TFT_YELLOW);
  ...
  myGLCD.drawString("About", 160, pgLine1); //GFXFF);
  myGLCD.setTextDatum(TL_DATUM);        // Left Justify Text (default)
  
  myGLCD.drawString("File:  " FILE_NAME, pgStart, pgLine2);  //GFXFF);
  myGLCD.drawString("Compiled:  " __DATE__ ", " __TIME__ ", " __VERSION__, pgStart, pgLine3);
  myGLCD.drawString(ideVer, pgStart, pgLine4);
  myGLCD.drawString("IP Address:   " + ipAddr, pgStart, pgLine5);
  myGLCD.drawString("MAC Address:  " + macAddr, pgStart, pgLine6);
  
  
  //myGLCD.drawString(WiFi.localIP().toString(), puFrameLeft + 20, pgLine5 + 10, GFXFF);
  //myGLCD.drawString(WiFi.macAddress(), puFrameLeft + 20, pgLine6 + 10, GFXFF);
...
void displayAlarmSetting()
{
  ...
  myGLCD.setTextSize(4);      // Text size multiplier
...
void displayWiFiMenu(void)
{
  ...
  myGLCD.setTextColor(TFT_WHITE, TFT_BLACK);
  myGLCD.setTextDatum(TC_DATUM);         // Center Text
  myGLCD.drawString("WiFi Settings", pgCenter, pgLine2, GFXFF);
